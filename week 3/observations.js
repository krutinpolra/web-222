...
function getTotalResults(data) {
  // checks if the passes value is the number not anything else
  // and will return the digit by accessing the property of the object
  return typeof data.total_results === 'number' ? data.total_results : false;
}
...
function speciesCoordinates(data) {
  ...
  for (let i = 0; i < results.length; i++) {
    // Format the string and log it to the console
    console.log(
...
function speciesCoordinates2(data) {
  // use of for each to print the results
...
function speciesCoordinates3(data) {
  //use of for of to print the results
...
function observationsByQualityGrade(data, qualityGrade) {
  // Check if qualityGrade is one of the expected values (case-insensitive)
  const allowedValues = ['research', 'needs_id', 'casual', null];
  qualityGrade = typeof qualityGrade === 'string' ? qualityGrade.toLowerCase() : qualityGrade;

  if (!allowedValues.includes(qualityGrade)) {
    throw new Error('Invalid qualityGrade value');
  }

  // Create an empty array to store matching observation objects
  const matchingElement = new Array();

  // Iterate over the data array and filter observations by quality_grade
  data.results.forEach((observation) => {
    if (observation.quality_grade === qualityGrade) {
      matchingElement.push(observation);
    }
  });
...
function observationsByQualityGrades(data, ...qualityGrades) {
  let matchingElement = new Array();
  qualityGrades.forEach((observation) => {
    observation = typeof observation === 'string' ? observation.toLowerCase() : observation;
    if (
      ...
      throw new Error('Invalid qualityGrades provided.');
    }
    data.results.forEach((element) => {
      if (element.quality_grade === observation) {
...
function transformObservation(original) {
  ...
  function getPhotos(photos) {
    let arrayOfPhotos = new Array();
    photos.forEach((image) => {
      arrayOfPhotos.push({ url: image.url, copyright: image.attribution });
    });
    return arrayOfPhotos;
  }

  function check(param, func = null, defaultValue = null) {
    if (param) {
      if (func) {
        return func(param);
      }
  ...
  return {
    id: check(original.id),
    name: check(original.species_guess, (string) => string.toLowerCase()),
    isExtinct: check(
      original.conservation_status,
      (conservationStatus) => conservationStatus.status_name === 'extinct in the wild',
      false
    ),
    images: check(original.photos, getPhotos, new Array()),
    observer: check(original.user, (userData) => `${userData.login_exact}@inaturalist.com`)
  };
}
...
function transformObservations(data) {
  let transformedObservation = new Array();
  for (const element of data.results) {
    transformedObservation.push(transformObservation(element));
  }
...
function getObservationsByTaxa(data, ...taxaNames) {
  return taxaNames.reduce((result, name) => {
    const observations = data.results.filter((element) => element.taxon.iconic_taxon_name === name);
...
function getObservationsByLocation(data, options = {}) {
  ...
  function observedLocationFilter(entry) {
    observedLocation = entry.location.split(',');
    if (options.lat) {
      if (options.lat.min) {
        return (
          observedLocation[0] >= options.lat.min &&
          observedLocation[0] <= options.lat.max &&
          observedLocation[1] >= options.lng.min &&
          observedLocation[1] <= options.lng.max
        );
      }

      return (
        parseFloat(observedLocation[0]) === options.lat &&
        parseFloat(observedLocation[1]) === options.lng
      );
    }
    return true;
  }
  return data.results.filter(observedLocationFilter);
}
...
function getPlaceURLs(data) {
  ...
  data.results.forEach((element) => {
    element.place_ids.forEach((id) => {
      uniquePlaceIds.add(id);
    });
  });

  const urls = Array.from(uniquePlaceIds).map(
    (id) => `https://www.inaturalist.org/observations?place_id=${id}`
  );
  return urls;
}
...
function extractSpeciesNames(data) {
  ...
  for (const element of data.results) {
    if (!speciesNamesArray.includes(element.species_guess)) {
      speciesNamesArray.push(element.species_guess);
    }
  }
...
function extractSpeciesNames2(data) {
  let extractSpeciesNamesArray = new Set();
  for (const element of data.results) {
    if (!extractSpeciesNamesArray.has(element.species_guess)) {
      extractSpeciesNamesArray.add(element.species_guess);
    }
  }

  return Array.from(extractSpeciesNamesArray);
}
